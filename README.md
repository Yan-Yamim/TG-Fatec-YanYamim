...
- **Estruturação de arquitetura e template base** <details> <summary>Ver detalhes</summary>  Criação da infraestrutura inicial da API utilizando o framework FastAPI. No back-end, aplicou-se o gerenciador de pacotes UV para otimização da resolução de dependências e instalação de bibliotecas correlatas (como SQLAlchemy, Pytest e Pandas). Para a interface do usuário (front-end), implementou-se uma arquitetura leve baseada em petit-vue (variação minimalista do Vue.js), viabilizando a reatividade via importação direta em script HTML, eliminando a sobrecarga de gerenciadores como Node.js ou NPM. </details>
