...
- **View page de exibição das Issues** <details> <summary>Ver detalhes</summary> Utilizando o framework **Vue.js** e a biblioteca de componentes **Vuetify**, desenvolvi uma interface reativa utilizando o componente <v-table>, integrado nativamente a um sistema de paginação assíncrona. O fluxo de dados consome a API do backend de forma não-bloqueante. Implementei uma estratégia de ambiente via **Docker** que permite a alternância transparente entre o consumo de uma API real e uma **API Mock**, facilitando o desenvolvimento frontend independente do status do backend. </details>
  
- **Documentação do SonnarQube** <details> <summary>Ver detalhes</summary> Atuando no fluxo de DevOps, fui responsável pela documentação técnica e implementação da análise estática de código via **SonarQube**. O escopo incluiu o setup da ferramenta via containerização, a definição de **Quality Gates**, a parametrização de scanners para visualização de cobertura de testes e a análise crítica de *Technical Debt* (Débito Técnico) e vulnerabilidades para garantir a integridade e segurança da aplicação.</details> 
...
- **CRUD de usuário** <details> <summary>Ver detalhes</summary> O fluxo de usuário se trata na criação de endpoints HTTP capazes de fazerem o `GET`, `POST`, `PUT` e `DELETE` de usuário. Usando componentes do FastApi e do SQLAlchemy foi possível estruturar uma API assíncrona robusta e segura, utilizando o decorator `@router` do FastAPI acoplado a um APIRouter sob o prefixo /users para mapear de forma idiomática os verbos HTTP correspondentes ao ciclo de vida completo do recurso. Na criação de um usuário via método `POST`, o fluxo valida regras de negócio cruciais, como a obrigatoriedade do consentimento dos termos e a unicidade de campos através de consultas, impedindo a duplicidade de nome de usuário ou e-mail antes de aplicar técnicas de segurança como a cifragem de senhas com `get_password_hash` e a geração de tokens seguros com a biblioteca secrets. O resgate de dados é segmentado em duas frentes com o GET: uma rota (/) mostra todos os usuários paginados, e a /me, retorna o perfil do usuário autenticado no escopo da requisição. Por fim, os métodos `PUT` e `DELETE`, implementam uma camada estrita de autorização que restringe a alteração ou exclusão da conta apenas ao próprio usuário autenticado através de uma verificação, disparando exceções de status proibido (HTTPStatus.FORBIDDEN) caso haja tentativa de acesso indevido, e integrando gatilhos pós-transação como o registro de logs de auditoria detalhados e funções auxiliares de expurgo do identificador do usuário.</details>

- **Sistema de autenticação de usuário** <details> <summary>Ver detalhes</summary> O fluxo de autenticação foi através de componentes do FastAPI e do SQLAlchemy para estruturar uma camada de segurança assíncrona baseada em tokens `JWT` (JSON Web Tokens). O gerenciamento de credenciais utiliza a biblioteca `pwdlib` por meio do `PasswordHash.recommended()`, estabelecendo as funções utilitárias para realizar o hash seguro e a verificação criptográfica de senhas, respectivamente, evitando o armazenamento de dados sensíveis em texto puro. A geração do token de acesso ocorre na função `create_access_token`, que anexa um tempo de expiração e codifica o payload utilizando uma chave secreta e algoritmos criptográficos definidos nas configurações globais do sistema. Caso o token seja considerado inválido, o sistema dispara imediatamente uma exceção HTTP de não autorizado `(HTTPStatus.UNAUTHORIZED)`. Superada a fase de decodificação do JWT, a dependência utiliza a sessão assíncrona do banco de dados para realizar uma consulta para buscar o usuário pelo e-mail recuperado do token, validando a existência do registro no banco antes de retornar a instância completa do usuário autenticado para as rotas dependentes.</details>
  
- **Cálculo do TAM** <details> <summary>Ver detalhes</summary> Um dos tratamentos de dados das concessionárias que precisamos fazer foi o cálculo do TAM, que consistia em montar um gráfico com a soma (em quilômetros) da extensão dos fios de média tensão que circulava em determinados trechos. Para fazer esse cálculo, criei um service para o TAM e a integrei na arquitetura de pipeline que usamos. Para funcionar essa pipeline acionamos a rota pipeline/trigger e o arquivo .gdb é instalado e criado um job_id para ser salvo no MongoDB e nesse id nós injetamos os cálculos e os gráficos nessa linha de monstagem. Com o cálculo feito, fiz o ranking em ordem decrescente do trecho com maior extensão para o de menor extensão, ao final disso é criado uma collectio no MongoDB com os resultados dos cálculos. Além do service, criei a task do celery do TAM para executar na pipeline a respectiva tarefa.</details>

- **Gráfico do TAM** <details> <summary>Ver detalhes</summary> Através da biblioteca `matplotlib`, criei uma task no celery que após o cálculo do TAM renderiza os resultados em um gráfico de barras onde o eixo X eram os nomes das cidades analisadas do arquivo .gdb e o eixo Y são o comprimento (COMP_KM) em ordem decrescente para compôr o relatório final do usuário </details>
...
- **Mudança do layout do relatório final** <details> <summary>Ver detalhes</summary> Inicialmente, o relatório final estava com uma aparẽncia pouco padronizada e os gráficos de tamanhos variados. Por isso mesmo, alterei as cores dos gráficos para condizerem com a logo Thunderstone e padronizei o tamanho dos gráficos para todos ficarem visíveis e ocuparam a página inteira, usando ferramentas da biblioteca ReportLab e correções de títulos nas tasks de renderizar gráficos. </details>
